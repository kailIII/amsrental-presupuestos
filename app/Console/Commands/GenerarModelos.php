...
class GenerarModelos extends Command {
    ...
    public function fire() {
        ...
        \File::makeDirectory(app_path('modelos_generados'));
        $tablas = \App\SchemaHelper\Table::getTablesCurrentDatabase();
        $this->info("Buscando tablas..");
        foreach ($tablas as $tabla) {
            $this->info("Generando Modelo de la tabla: " . $tabla->table_name);
            //class name
            $class_name = ucfirst(camel_case(Helper::strSingularSpanish($tabla->table_name)));
            $baseString = \File::get(app_path('SchemaHelper/Template.txt'));
            //replace class name..
    ...
    private function generarBelongsTo($columns) {
        ...
        foreach ($columns as $column) {
            ...
            if (ends_with($column->column_name, '_id')) {
                $func = \File::get(app_path('SchemaHelper/BelongsTo.txt'));
